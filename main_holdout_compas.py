...
DATA_FOLDER = "data/COMPAS/holdout/recidivism_%s.csv"


def prepair_data(cluster_index):
    train_df = pd.read_csv(DATA_FOLDER % ("train_%d" % cluster_index))
    ...
    test_df = pd.read_csv(DATA_FOLDER % "test")
    ...
    test_index_df = pd.read_csv(DATA_FOLDER % ("test_index_%d" % cluster_index), header=None)
    test_index = test_index_df.iloc[:,0].tolist()
    
    return (train_loader, train_test_loader, val_loader, test_loader, train_data, train_test_data, val_data, test_data, y_train, y_val, y_test, test_index)

def experiment(data, cluster_index, epoch, lr, hidden):

    (train_loader, train_test_loader, val_loader, test_loader, train_data, train_test_data, val_data, test_data, y_train, y_val, y_test, test_index) = data
    ...
    for run in range(NUM_RUNS):
        ...
        if os.path.isfile(run_pred_file):
            ...
            if os.path.isfile(run_model_file):
                ...
                model = simple_binary.train(train_loader, 13, epoch=epoch, lr=lr, hidden=hidden)
        ...
        test_neg_index = np.array(list(range(len(y_test))))
        test_neg_index = np.delete(test_neg_index, test_index)
        test_holdout_report = process([test_conf[i] for i in test_index], [y_test[i] for i in test_index])
        test_normal_report = process([test_conf[i] for i in test_neg_index], [y_test[i] for i in test_neg_index])
        ...
        print_report(test_holdout_report[1], test_holdout_report[2], f)
        print_report(test_normal_report[1], test_normal_report[2], f)
...
for index in range(15):
    data = prepair_data(index)
    #experiment(data,a,10,0.001,64)
    experiment(data,index,10,0.0001,64)
    #experiment(data,a,1000,0.0001,64)
    #experiment(data,a,1000,0.0001,128)


#experiment(data, 1, 0.001, 64)
